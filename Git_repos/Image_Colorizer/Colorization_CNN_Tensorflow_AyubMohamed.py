...
# DISCLAIMER: I disabled TensorFlow 2.x's eager execution to allow use of TensorFlow 1.x-style graphs. 
# I did this because I was working on the basis of Tensorflow 1.x knowledge, instead of rewriting everthing I decided to make my code compatible with Tensorflow 2.x.
# Some tensorflow methods will be called with compat.v1. to allow compatibility

# ==== Configuration of image directories\ replace whatever is in the quatations with your own directory ====
...
def load_and_preprocess_images(directory, max_images=None):
    ...
    X_L = lab_data[:, :, :, 0] / 100.0  
    Y_ab = (lab_data[:, :, :, 1:] + 128) / 255.0  
    return X_L[..., np.newaxis], Y_ab, files

# ==== Load data ====
X_train, Y_train, train_files = load_and_preprocess_images(train_dir, max_images=20)
X_test, Y_test, test_files = load_and_preprocess_images(test_dir, max_images=20)
...
with tf.compat.v1.variable_scope('decoder'):
    ...
    output = tf.nn.tanh(output_layer) * 0.5 + 0.5
...
def safe_lab2rgb(L, ab):
    ...
    lab[:, :, 1:] = np.clip(ab * 255.0 - 128, -128, 127)  
